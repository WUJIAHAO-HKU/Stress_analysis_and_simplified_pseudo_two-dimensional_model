...
function sP2D_potentials(case::Case, yt::Array{Float64}, t::Float64, variables::Dict{String, Union{Array{Float64},Float64}})
    ...
    eta_n_gs_rel = phis_n .- phie_n_gs_rel - u_n_gs - (2/3) * stress_theta_n_surf_gs * case.param.NE.Omega
    eta_p_gs_rel = phis_p0 .- phie_p_gs_rel - u_p_gs - (2/3) * stress_theta_p_surf_gs * case.param.PE.Omega
    ...
    for i = 1:3
        phie
        phie_old = phie
        eta_n_gs = phis_n .- phie_n_gs_rel - u_n_gs
        j_n_gs = j0_n_gs .* sinh.(0.5 * eta_n_gs / T) * 2.0
        # 更新 phie0 和 phie
        phie = phie_fit(mesh_el.node, Li, ki, phie0)
        if norm(phie - phie_old) < 1e-6
            break
        end
    end
    ...
    if typef == "sin"
        for i in eachindex(x)
            if x[i] <= Ln
                ...
                t = x[i]/Ln
                # 添加三次项来提供更好的曲率控制
                phie[i] = kn * (0.5 * t^2 - t^3/3) - kn * Ln/6 - ks * Ls/2 + phie0

                # 负极-隔膜界面过渡区（x ∈ [Ln-δ, Ln+δ]）
            elseif Ln-m ≤ x[i] ≤ Ln+m
                t = x[i]/Ln  # 负极侧归一化坐标
                a = (x[i] - (Ln-m)) / (2m)  # 过渡权重
                phie_left = kn*(0.5*t^2 - t^3/3) - kn*Ln/6 - ks*Ls/2 + phie0
                phie_right = ks*(x[i]-Ln-Ls/2) + phie0
                phie[i] = (1-a)*phie_left + a*phie_right + 0.08*ks*Ls*(α²-a)
            ...
            elseif x[i] < Ln + Ls
                ...
                phie[i] = ks * (x[i] - Ln - Ls/2) + 
                         0.08 * ks * Ls * (t^2 - t) + phie0
            elseif Ln + Ls - m ≤ x[i] ≤ Ln + Ls + m
            # 过渡区坐标定义
            a = (x[i] - (Ln + Ls - m)) / (2m)  # α ∈ [0,1]
                            
            # 隔膜侧电势（左）
            t_sep = (x[i] - Ln)/Ls
            phie_left = ks*(x[i] - Ln - Ls/2) + 0.08*ks*Ls*(t_sep^2 - t_sep) + phie0
                            
            # 正极侧电势（右）
            t_p = (L - x[i])/Lp
            phie_right = -kp*(0.5*t_p^2 - t_p^3/3) + kp*Lp/6 + ks*Ls/2 + phie0
                            
            # 加权平均 + 平滑项
            phie[i] = (1-a)*phie_left + a*phie_right + 0.08*kp*Lp*(a^2 - a)

            else
                # 正极区域：改进的二次函数
                t = (L - x[i])/Lp
                # 添加三次项来提供更好的曲率控制
                phie[i] = -kp * (0.5 * t^2 - t^3/3) + kp * Lp/6 + ks * Ls/2 + phie0
            end
        end
    elseif typef == "quad"
        pi = acos(-1.0)
...
function sP2D_variables(case::Case, yt::Array{Float64}, t::Float64)
    ...
    if case.opt.mechanicalmodel == "full"
        # 考虑应力对开路电压的影响
        stress_factor_n = (2/3) * variables["negative particle surface tangential stress"] * case.param.NE.Omega
        stress_factor_p = (2/3) * variables["positive particle surface tangential stress"] * case.param.PE.Omega
        u_n = u_n + stress_factor_n
        u_p = u_p + stress_factor_p
    end
