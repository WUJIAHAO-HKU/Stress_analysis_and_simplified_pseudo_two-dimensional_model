    ...
    if typef == "sin"
        for i in eachindex(x)
            if x[i] <= Ln
                # 负极区域：添加五阶项并微调系数
                t = x[i]/Ln
                phie[i] = kn * (0.5*t^2 - 0.3445*t^3 + 0.106*t^4 - 0.021*t^5) - 
                         kn * Ln/2 - ks * Ls/2 + phie0
                
                # 改进边界层修正
                if x[i] >= Ln - δn
                    β = (x[i] - (Ln - δn))/δn
                    correction = 0.1575 * kn * δn * (
                        β^2 * (3 - 2β) + 
                        0.1225*β*(1 - β)*(1 - 2β) +
                        0.018*β^2*(1 - β)^2
                    )
            ...
            elseif x[i] < Ln + Ls
                ...
                t = (x[i] - Ln)/Ls
                base = ks * (x[i] - Ln - Ls/2)
                transition = t^2 * (3 - 2t) + 0.1225*t*(1 - t)*(1 - 2t) + 
                           0.018*t^2*(1 - t)^2
                correction = 0.1575 * ks * Ls * (transition - 0.5)
                phie[i] = base + correction + phie0
                
            else
                # 正极区域：添加五阶项并微调系数
                t = (x[i] - (Ln + Ls))/Lp
                phie[i] = -kp * (0.5*t^2 - 0.3445*t^3 + 0.106*t^4 - 0.021*t^5) + 
                         kp * Lp/2 + ks * Ls/2 + phie0
                
                # 改进边界层修正
                if x[i] <= Ln + Ls + δp
                    β = (x[i] - (Ln + Ls))/δp
                    correction = 0.1575 * kp * δp * (
                        β^2 * (3 - 2β) + 
                        0.1225*β*(1 - β)*(1 - 2β) +
                        0.018*β^2*(1 - β)^2
                    )
    ...
    elseif typef == "quad"
        ...
        for i in eachindex(x)
            if x[i] <= Ln
                ...
                if x[i] >= Ln - δn
                    β = (x[i] - (Ln - δn))/δn
                    correction = 0.158 * kn * δn * (
                        sin(pi*β) + 
                        0.1225*sin(2*pi*β)*(1 - β) +
                        0.018*sin(3*pi*β)*(1 - β)^2
                    )
                    phie[i] += correction
                end
            ...
            elseif x[i] < Ln + Ls
                ...
                transition = 0.5 * (1 - cos(pi*t)) + 
                           0.0625 * sin(2*pi*t)*(1 - t)*t +
                           0.018 * sin(3*pi*t)*(1 - t)^2*t
                correction = 0.1575 * ks * Ls * (transition - 0.5)
                phie[i] = base + correction + phie0
