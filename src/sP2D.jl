...
function sP2D_potentials(case::Case, yt::Array{Float64}, t::Float64, variables::Dict{String, Union{Array{Float64},Float64}})
    ...
    # 构建 yt_new，需要包含所有非电势变量（粒子、电解液浓度、温度等）
    n_non_potential = mesh_np.nlen + mesh_pp.nlen + mesh_el.nlen
    # 如果启用热模型，还需要包含温度
    if case.opt.thermalmodel == "lumped"
        n_non_potential += 1
    end
    yt_new = [yt[1:n_non_potential,1]; phi_new]
...
function sP2D_variables(case::Case, yt::Array{Float64}, t::Float64)
    param = case.param
    # 处理 yt 的维度：如果是2D矩阵（列向量），取第一列
    if ndims(yt) == 2 && size(yt, 2) == 1
        yt_vec = yt[:, 1]  # 取第一列作为向量
    else
        yt_vec = yt
    end
    ...
    for i in var_list
        variables[i] = yt_vec[case.index[i]] # hcat converts vector to matrix
    end
    if "temperature" in var_list
        T_val = yt_vec[case.index["temperature"]]
        # 确保 T 是标量：如果是向量则取第一个值（或平均值）
        if length(T_val) == 1
            T = T_val[1]
        elseif isa(T_val, AbstractArray)
            T = sum(T_val) / length(T_val)  # 计算平均值
        else
            T = T_val
        end
    else
