...
for j =1:4
    ...
    # 获取负极第一个位置的颗粒表面切向应力
    # 注意：负极表面切向应力是一个矩阵，第一个位置对应的是[1,:]
    neg_surf_stress_sp2d = result["negative particle surface tangential stress[Pa]"][1,:]
    neg_surf_stress_p2d = result2["negative particle surface tangential stress[Pa]"][1,:]
    
    # 获取时间数据
    time_sp2d = result["time [s]"]
    time_p2d = result2["time [s]"]

    # 计算SP2D和P2D表面应力的平均值（用于比较）
    neg_surf_stress_avg = calculate_average_data(
        time_sp2d, neg_surf_stress_sp2d,
        time_p2d, neg_surf_stress_p2d
    )

    # 绘制结果，改为时间和切向应力的关系
    plot!(p, time_p2d, neg_surf_stress_p2d, 
          label="P2D-$(Crate)C", 
          xlabel="Time [s]", 
          ylabel="Stress [Pa]", 
          ...
          linestyle=:solid,
          linewidth=2)
    plot!(p, time_sp2d, neg_surf_stress_avg, 
          label="sP2D-$(Crate)C", 
          linecolor=colors[j],
          linestyle=:dash, 
          linewidth=2.5)
end
    savefig(p, "负极表面切向应力比较_LG_M50.pdf")
