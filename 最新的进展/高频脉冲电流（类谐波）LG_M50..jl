...
time_step = 0.01        # 时间步长 (10ms，捕捉高频脉冲细节)
...
plot!(time_result, voltage_sP2D, 
    label="voltage (sP2D)", 
    title="voltage_comparison (LG M50)",
...
# 添加RMSE标注
annotate!(500, maximum(voltage_p2d) - 0.15,  # 调整位置到图表上方
...
# 标记从空间分布中选择的三个时间点
analysis_times = [40.3, 504.0, 1000.2]  # 与空间分布文件中相同的时间点

# 为每个分析时间点添加垂直虚线和红色交叉标记
for t in analysis_times
    # 添加垂直虚线
    vline!([t], color=:gray, linestyle=:dash, alpha=0.7, linewidth=1.5, label="")
    
    # 获取该时间点对应的电流值
    idx = argmin(abs.(time .- t))
    current_at_t = current[idx]
    
    # 添加红色交叉标记
    scatter!([t], [current_at_t], marker=:xcross, color=:red, 
             markersize=12, markerstrokewidth=3, label="")
end

# 为图例添加一个红色交叉标记样本
scatter!([NaN], [NaN], marker=:xcross, color=:red, 
         markersize=12, markerstrokewidth=3, label="Analysis Time Points")
...
# 在零线旁边添加文本标注
annotate!(pulse_width*500, 0.01, text("zero reference", :right, 8, :black))
...
annotate!(pulse_width*500-180, maximum(noise_data./I_base)*0.8,  # 下移 0.2C
...
p_cycle = plot(cycle_time .* 1000, cycle_current ./ I_base,  # 转换为毫秒显示，电流转换为倍率C
    xlabel="Time [ms]", 
    ylabel="Current [C]", 
    title="three_stage_current_cycle",
    lw=2, 
    color=:blue,
    dpi=600,
    size=(800, 400),
...
# 3. 组合图：噪声和电流周期对比 (改为 1x2 横向布局)
# 若仍需要竖排布局，可将下面 layout 改回 (2,1)
...
savefig(p_combined_new, "pulse_noise_and_current_cycle_combined_horizontal.png")
