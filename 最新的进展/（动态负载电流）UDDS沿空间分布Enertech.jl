...
# 定义科学论文风格的配色方案
# 根据要求，使用固定的两种颜色，通过透明度区分不同时间点
# 负极使用蓝紫色，正极使用橙红色
p2d_color = :blueviolet  # 负极颜色 - 蓝紫色
sp2d_color = :orangered   # 正极颜色 - 橙红色
...
annotate!(neg_center_conc, maximum([maximum(conc) for conc in concentration_p2d_spatial])-0.33, 
         text("Negative\nElectrode", 9, :center, :bold))
annotate!(pos_center_conc, maximum([maximum(conc) for conc in concentration_p_p2d_spatial])*1.10, 
         text("Positive\nElectrode", 9, :center, :bold))
...
annotate!(neg_center_stress, maximum([maximum(stress) for stress in stress_p2d_spatial])-0.00035, 
         text("Negative\nElectrode", 9, :center, :bold))
annotate!(pos_center_stress, maximum([maximum(stress) for stress in stress_p_p2d_spatial])+ 0.0014, 
         text("Positive\nElectrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(stress) for stress in stress_p2d_spatial])*0.57, 
         text("Separator", 9, :center, :italic, :gray))
...
annotate!(neg_center_reaction, maximum([maximum(curr) for curr in reaction_current_p2d_spatial])*0.60, 
         text("Negative\nElectrode", 9, :center, :bold))
annotate!(pos_center_reaction, maximum([maximum(curr) for curr in reaction_current_p_p2d_spatial])+ 2.5, 
         text("Positive\nElectrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(curr) for curr in reaction_current_p2d_spatial])- 1.5, 
         text("Separator", 9, :center, :italic, :gray))
...
annotate!(neg_center_over, maximum([maximum(over) for over in overpotential_p2d_spatial])-0.20, 
         text("Negative\nElectrode", 9, :center, :bold))
annotate!(pos_center_over, maximum([maximum(over) for over in overpotential_p_p2d_spatial])+0.03, 
         text("Positive\nElectrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(over) for over in overpotential_p2d_spatial])*0.38, 
         text("Separator", 9, :center, :italic, :gray))
...
for (param, value) in overall_rmse
    println("| ", rpad(param, 10), " | ", round(value, digits=20))
end
println("-"^40)
...
println("  - 固相电导率 (sig): ", param_dim.NE.sig, " S/m")
println("  - 电极厚度 (thickness): ", param_dim.NE.thickness, " m")
println("  - 固相扩散时间尺度 (τ_diff = rs^2 / Ds): ", τ_diff_n, " s")

# 估算固相电导时间尺度 (τ_cond)
C_dl = 20e-6  # 双电层电容率的典型值 [F/m^2]
τ_cond_n = (C_dl * param_dim.NE.as * param_dim.NE.thickness^2) / (param_dim.NE.sig * param_dim.NE.eps_s)
println("  - 固相电导时间尺度 (τ_cond): ", τ_cond_n, " s")
...
println("  - 固相电导率 (sig): ", param_dim.PE.sig, " S/m")
println("  - 电极厚度 (thickness): ", param_dim.PE.thickness, " m")
println("  - 固相扩散时间尺度 (τ_diff = rs^2 / Ds): ", τ_diff_p, " s")
# 估算固相电导时间尺度 (τ_cond)
C_dl = 20e-6  # 双电层电容率的典型值 [F/m^2]
τ_cond_p = (C_dl * param_dim.PE.as * param_dim.PE.thickness^2) / (param_dim.PE.sig * param_dim.PE.eps_s)
println("  - 固相电导时间尺度 (τ_cond): ", τ_cond_p, " s")
println("------------------------------------------\n")