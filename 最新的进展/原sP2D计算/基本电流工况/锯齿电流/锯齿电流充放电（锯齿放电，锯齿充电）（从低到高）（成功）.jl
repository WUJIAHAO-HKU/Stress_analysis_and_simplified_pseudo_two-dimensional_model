...
opt.Current = (t) -> begin
    if t < charge_time
        # 放电阶段：周期性上升锯齿波
        return charge_amplitude * ((t % charge_period) / charge_period)
    else
        # 充电阶段：周期性上升锯齿波
        return discharge_amplitude * ((t % discharge_period) / discharge_period)
    end
end
...
# 图形修饰
xlims!(0, total_time)
title!("Sawtooth Charging → Sawtooth Discharging(up)")
savefig("Sawtooth_charge_sawtooth_discharge(low to high).pdf")
