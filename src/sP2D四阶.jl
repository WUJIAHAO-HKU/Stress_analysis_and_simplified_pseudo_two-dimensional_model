    ...
    if typef == "sin"
        for i in eachindex(x)
            if x[i] <= Ln
                # 负极区域：微调四阶多项式系数
                t = x[i]/Ln
                phie[i] = kn * (0.5*t^2 - 0.345*t^3 + 0.106*t^4) - kn * Ln/2 - ks * Ls/2 + phie0
                
                # 边界层修正
                if x[i] >= Ln - δn
                    β = (x[i] - (Ln - δn))/δn
                    correction = 0.158 * kn * δn * (β^2 * (3 - 2β) + 0.122*β*(1 - β)*(1 - 2β))
                    phie[i] += correction
                end
            ...
            elseif x[i] < Ln + Ls
                ...
                t = (x[i] - Ln)/Ls
                base = ks * (x[i] - Ln - Ls/2)
                transition = t^2 * (3 - 2t) + 0.122*t*(1 - t)*(1 - 2t)
                ...
                # 正极区域：微调四阶多项式系数
                t = (x[i] - (Ln + Ls))/Lp
                phie[i] = -kp * (0.5*t^2 - 0.345*t^3 + 0.106*t^4) + kp * Lp/2 + ks * Ls/2 + phie0
                
                # 边界层修正
                if x[i] <= Ln + Ls + δp
                    β = (x[i] - (Ln + Ls))/δp
                    correction = 0.158 * kp * δp * (β^2 * (3 - 2β) + 0.122*β*(1 - β)*(1 - 2β))
                    phie[i] += correction
                end
            end
    ...
    elseif typef == "quad"
        ...
        for i in eachindex(x)
            if x[i] <= Ln
                ...
                if x[i] >= Ln - δn
                    β = (x[i] - (Ln - δn))/δn
                    correction = 0.158 * kn * δn * (sin(pi*β) + 0.122*sin(2*pi*β)*(1 - β))
                    phie[i] += correction
                end
