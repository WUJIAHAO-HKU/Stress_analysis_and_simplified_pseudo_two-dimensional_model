...
# 选择3个关键时间点，时间越后颜色越深
analysis_times = [40.3, 504.0, 1000.2]
time_indices = []
...
annotate!(neg_center_conc, maximum([maximum(conc) for conc in concentration_p2d_spatial])*0.95, 
...
annotate!(neg_center_stress, maximum([maximum(stress) for stress in stress_p2d_spatial])*1.05, 
         text("Negative\nElectrode", 9, :center, :bold))
annotate!(pos_center_stress, maximum([maximum(stress) for stress in stress_p_p2d_spatial])*0.75, 
         text("Positive\nElectrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(stress) for stress in stress_p2d_spatial])*0.5, 
         text("Separator", 9, :center, :italic, :gray))
...
annotate!(pos_center_reaction, maximum([maximum(curr) for curr in reaction_current_p_p2d_spatial])*0.1, 
         text("Positive\nElectrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(curr) for curr in reaction_current_p2d_spatial])*0.15 - 0.7, 
         text("Separator", 9, :center, :italic, :gray))
...
annotate!(neg_center_over, maximum([maximum(over) for over in overpotential_p2d_spatial])*0.95, 
         text("Negative\nElectrode", 9, :center, :bold))
annotate!(pos_center_over, maximum([maximum(over) for over in overpotential_p_p2d_spatial])*0.05, 
         text("Positive\nElectrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(over) for over in overpotential_p2d_spatial])*0.25, 
         text("Separator", 9, :center, :italic, :gray))
...
for i in 1:length(time_indices)
    ...
    scatter!(x_grid_p_normalized, potential_p_sP2D_spatial[i], 
        label="", 
        markersize=5, color=time_colors[i], marker=:star5, alpha=0.8)
...
annotate!(neg_center_conc_err, maximum([maximum(err) for err in concentration_spatial_errors_n])*0.95, 
         text("Negative Electrode", 9, :center, :bold))
annotate!(pos_center_conc_err, maximum([maximum(err) for err in concentration_spatial_errors_p])*1.25, 
         text("Positive Electrode", 9, :center, :bold))
...
annotate!(neg_center_stress_err, maximum([maximum(err) for err in stress_spatial_errors_n])*1.1, 
         text("Negative Electrode", 9, :center, :bold))
annotate!(pos_center_stress_err, maximum([maximum(err) for err in stress_spatial_errors_p])*0.7, 
         text("Positive Electrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(err) for err in stress_spatial_errors_n])*1.2, 
         text("Separator", 8, :center, :italic, :gray))
...
annotate!(neg_center_reaction_err, maximum([maximum(err) for err in reaction_current_spatial_errors_n])*2.0, 
         text("Negative Electrode", 9, :center, :bold))
annotate!(pos_center_reaction_err, maximum([maximum(err) for err in reaction_current_spatial_errors_p])*0.85, 
         text("Positive Electrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(err) for err in reaction_current_spatial_errors_n])*2.2, 
         text("Separator", 8, :center, :italic, :gray))
...
annotate!(neg_center_over_err, maximum([maximum(err) for err in overpotential_spatial_errors_n])*2.3, 
         text("Negative Electrode", 9, :center, :bold))
annotate!(pos_center_over_err, maximum([maximum(err) for err in overpotential_spatial_errors_p])*0.8, 
         text("Positive Electrode", 9, :center, :bold))
annotate!(separator_mid, maximum([maximum(err) for err in overpotential_spatial_errors_n])*2.2, 
         text("Separator", 8, :center, :italic, :gray))
