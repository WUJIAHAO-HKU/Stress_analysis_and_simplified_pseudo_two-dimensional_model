...
# 添加RMSE标注
annotate!(500, maximum(voltage_p2d) - 0.77,  # 调整位置往下移动
...
# 创建电流图 - 使用黑色曲线并标注10个时间点
# 选择UDDS周期中的十个平均分隔点（与沿空间分布文件中相同）
...
p4 = plot(time_udds, current_udds, 
    ...
    color=:black, 
    dpi=600,
    titlefontsize=40,                # 大号标题字体
    guidefontsize=36,                # 轴标签字体大小
    tickfontsize=32,                 # 刻度字体大小
    legendfontsize=26,               # 图例字体大小
    grid=false,                       # 添加网格线以提高可读性
    gridalpha=0.3,                   # 网格线透明度
    left_margin=10Plots.mm,          # 左边距，为Y轴标签留出空间
    bottom_margin=10Plots.mm)        # 底部边距，为X轴标签留出空间

# 为每个分析时间点添加垂直虚线和红色交叉标记
for t in analysis_times
    # 添加垂直虚线
    vline!([t], color=:gray, linestyle=:dash, alpha=0.6, linewidth=1.5, label="")
    
    # 获取该时间点对应的电流值
    current_at_t = current_interp(t)
    
    # 添加红色交叉标记
    scatter!([t], [current_at_t], marker=:xcross, color=:red, 
             markersize=10, markerstrokewidth=2.5, label="")
end

# 为图例添加一个红色交叉标记样本
scatter!([NaN], [NaN], marker=:xcross, color=:red, 
         markersize=10, markerstrokewidth=2.5, label="Analysis Points")
...
p4b = plot(time_zoom, current_zoom, 
    ...
    lw=3,                        # 增加线宽
    size=(1600, 400), 
    color=:black,                # 保持与主电流图相同的黑色
    titlefontsize=22,            # 大号标题字体
    guidefontsize=18,            # 轴标签字体大小
    tickfontsize=16,             # 刻度字体大小
    legendfontsize=16,           # 图例字体大小
    grid=true,                   # 添加网格线以提高可读性
    gridalpha=0.3,               # 网格线透明度
    framestyle=:box,             # 添加边框
    left_margin=10Plots.mm,      # 左边距，为Y轴标签留出空间
...
# 添加RMSE标注
annotate!(70, minimum(voltage_p2d_zoom) + 0.15,  # x坐标从50改为70，y坐标偏移从0.2改为0.1
